...
extern "C" {
	...
	TbBool detonate_shot(struct Thing *shotng)
	{
		...
		if (shotst->area_range != 0) {
			...
			long dist, damage;
			dist =  saturate_set_signed(shotst->area_range*COORD_PER_STL, 16);//compute_creature_attack_range(shotst->area_range*COORD_PER_STL, crstat->luck, cctrl->explevel);
			SYNCDBG(8,"RANGE-NORMAL:%d RANGE-SIGNED:%d",saturate_set_signed(dist,16), dist);
			
			damage = compute_creature_attack_spell_damage(shotst->area_damage, crstat->luck, cctrl->explevel);
			SYNCDBG(8,"DAMAGE-po %d",damage);
			...
			if ((blocked_flags & (SlbBloF_WalledX|SlbBloF_WalledY)) != 0)
			{
				...
				if(shotng->model == ShM_Grenade)
				{
					if(shotng->shot.dexterity >= ACTION_RANDOM(90)) 
					{
						...
						if(dist <= 800) shot_explodes = 1;
					}
				}
				if((shotst->model_flags & ShMF_Bounce) != 0)
				{
	...
	*/
	...
	long shot_hit_creature_at(struct Thing *shotng, struct Thing *trgtng, struct Coord3d *pos)
	{
		...
		if (shot_is_boulder(shotng))
		{
			...
			long max_dist1 = 500*COORD_PER_STL;
		//	long max_dist2 = 500*COORD_PER_STL;
		   int d1 = get_radially_decaying_value(100,max_dist1/4,3*max_dist1/4,500*COORD_PER_STL)+1;
		   int d2 = get_radially_decaying_value(100,max_dist1/4,3*max_dist1/4,100*COORD_PER_STL)+1;

			WARNDBG(8,"testDist 500-dmg:%d 100-dmg:%d",d1,d2);
		if (shotst->old->hit_sound != 0)
		{
		...
		if (shotng->shot.damage != 0)
		{
			...
			int divider = 1;
			if(creature_affected_by_spell(trgtng, SplK_Armour) && (shotng->model == ShM_Lightning || shotng->model == ShM_Drain)) divider = 5;

			if (!thing_is_invalid(shooter)) {


				apply_damage_to_thing_and_display_health(trgtng, shotng->shot.damage/divider, shotst->damage_type, shooter->owner);
			} else {
				apply_damage_to_thing_and_display_health(trgtng, shotng->shot.damage/divider, shotst->damage_type, -1);
			}
		}
		if (shotst->old->field_24 != 0)
		{
		...
		if (shotst->old->cast_spell_kind != 0)
		{
			...
			apply_spell_effect_to_thing(trgtng, shotst->old->cast_spell_kind, n, true);
		}
		...
		if (shotst->old->push_on_hit != 0)
		{
				...
				if(!creature_affected_by_spell(trgtng, SplK_Freeze))
				{
					WARNDBG(8,"Naboj:%s ph:%d target_freeze:%d",thing_model_name(shotng),shotst->old->push_on_hit,creature_affected_by_spell(trgtng, CCSpl_Freeze));

					i = shotst->old->push_on_hit * (long)shotng->velocity.x.val;
					trgtng->veloc_push_add.x.val += i / 16;
					i = shotst->old->push_on_hit * (long)shotng->velocity.y.val;
		...
		//if ((shotst->model_flags & ShMF_Detonate) != 0)
		if (shotst->area_range != 0 && shotng->model != 30 && shotng->model != 31)
	...
	*/
	...
	TngUpdateRet update_shot(struct Thing *thing)
	{
		...
		} else
		{
			...
			if ((shotst->model_flags & ShMF_Navigable) != 0){
			...
			case ShM_Firebomb:
				...
				break;
			case ShM_Lightning:
				if (((game.play_gameturn - thing->creation_turn) % 2) == 0 || game.play_gameturn - thing->creation_turn == 0)
				{
	/*
					target = thing_get(thing->shot.target_idx);
					
					struct Thing *enemy;
					enemy = enemy_near(shooter, thing->mappos.x.val,thing->mappos.y.val, 1);

					if (enemy != target) hit = true;
					*/
					...
					draw_lightning(&target->mappos,&shooter->mappos, 96, 60);

					if(dist > 5 && target->health>0 && shooter->health>0) hit = true;
					if((target->alloc_flags & TAlF_IsInLimbo) != 0 || (target->state_flags & TF1_InCtrldLimbo) != 0)  hit = true;
					if((shooter->alloc_flags & TAlF_IsInLimbo) != 0 || (shooter->state_flags & TF1_InCtrldLimbo) != 0)  hit = true;
					if(game.play_gameturn - thing->creation_turn > shotst->health/2)
					{
						if (ACTION_RANDOM(100) > 80) hit = true;
					}
				}
				...
				thing->move_angle_xy = (thing->move_angle_xy + LbFPMath_PI/9) & LbFPMath_AngleMask;
						...
						MapCoordDelta dist;
							...
							else 
							{
								...
								if(dist <= 260) hit = true;
								//show_onscreen_msg(game.num_fps,"turn: %d DISTMAX: %d", turn,DISTMAX);
							}
			...
			if ((shotst->model_flags & ShMF_Jumping) != 0){
