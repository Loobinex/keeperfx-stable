...
TbBool shot_hit_wall_at(struct Thing *shotng, struct Coord3d *pos)
{
    ...
    if ( !shot_explodes )
    {
        ...
        {
            if (shotng->model == ShM_Grenade)             {                 if (shotng->shot.dexterity >= ACTION_RANDOM(90))                 {                     struct Coord3d target_pos;                     target_pos.x.val = shotng->price.number;                     target_pos.y.val = shotng->shot.byte_19 * 300;                     target_pos.z.val = pos->z.val;                     const MapCoordDelta dist = get_2d_distance(pos, &target_pos);                     if (dist <= 800) return detonate_shot(shotng);                 }             }
            doortng = get_door_for_position(pos->x.stl.num, pos->y.stl.num);
            if (!thing_is_invalid(doortng))
            {
                efftng = create_shot_hit_effect(&shotng->mappos, shotng->owner, shotst->old->hit_door.effect_model, shotst->old->hit_door.sndsample_idx, shotst->old->hit_door.sndsample_range);
...
TngUpdateRet update_shot(struct Thing *thing)
{
    ...
    } else
    {
        ...
        case ShM_Grenade:
            ...
            int skill = thing->shot.dexterity;             target = thing_get(thing->shot.target_idx);             if (thing_is_invalid(target)) break;             MapCoordDelta dist;             if (skill <= 35)             {                 dist = get_2d_distance(&thing->mappos, &target->mappos);                 if (dist <= 260) hit = true;             }             else             {                 target_pos.x.val = thing->price.number;                 target_pos.y.val = thing->shot.byte_19 * 300;                 target_pos.z.val = target->mappos.z.val;                 dist = get_2d_distance(&thing->mappos, &target_pos);                 if (dist <= 260) hit = true;             }
            break;
        case ShM_Boulder:
